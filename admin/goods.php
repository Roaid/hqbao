...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    create_html_editor('goods_desc', $goods['goods_desc']);
 	$taobaourl= $goods['detail_url'];
    /* 模板赋值 */
    $smarty->assign('code',    $code);
	 $smarty->assign('taobaourl',    $taobaourl);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$tb_nick = empty($_POST['tb_nick']) ? '' : $_POST['tb_nick'];
	$tb_iid = empty($_POST['tb_iid']) ? '' : $_POST['tb_iid'];
	$tb_num_iid = empty($_POST['tb_num_iid']) ? '' : $_POST['tb_num_iid'];
	$detail_url = empty($_POST['detail_url']) ? '' : $_POST['detail_url'];
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,detail_url,tb_nick,tb_num_iid,tb_iid)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$detail_url', '$tb_nick', '$tb_num_iid', '$tb_iid')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$detail_url', '$tb_nick', '$tb_num_iid', '$tb_iid')";
        }
    }
    else
    {
...
else
{
	...
	//$taobaoURL="http://item.taobao.com/auction/item_detail-db1-8fd56f03fcc43e3a655364cfc7cf3643.htm";
	...
	if(!empty($shangp))
	{
		...
		$Taoapi->fields = 'iid,detail_url,num_iid,title,nick,pic_url,price,detail_url,freight_payer,express_fee';
		...
		$result['item_get_response']['item']['price']=$result['item_get_response']['item']['price']+$result['item_get_response']['item']['express_fee'];
...
function GrabImage($url,$filename="") {
	...
	if($filename=="") 
	{
		...
		fwrite($fp2,$img);
		fclose($fp2);
		return "http://www.hqbao.com/".$filename;
	}
?>
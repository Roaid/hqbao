...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    while ($row = $db->fetchRow($res))
    {
		...
		$row['goods_zt']=$_LANG['tb'][$row['goods_status']];
    ...
    elseif ('edit_goods' == $step)
    {
        if (isset($_POST['rec_id']))
        {
            foreach ($_POST['rec_id'] AS $key => $rec_id)
            {
				...
				$goods_g_info = $_POST['goods_g_info'][$key];
				$goods_zl = intval($_POST['goods_zl'][$key]);
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    elseif ('goods' == $step)
    {
		...
		/* 取得商品总金额 */
        $smarty->assign('goods_weight', order_weight($order_id));
    }
...
elseif ($_REQUEST['act'] == 'process')
{
    ...
    if ('taobaodg_order_goods' == $func)
    {
        ...
        /* 删除 */
        $sql = "update " . $ecs->table('order_goods') .
                "set goods_status=2 WHERE rec_id = '$rec_id' LIMIT 1";
        $db->query($sql);
    ...
    if ('taobaodh_order_goods' == $func)
    {
        ...
        $sql = "update " . $ecs->table('order_goods') .
                "set goods_status=3 WHERE rec_id = '$rec_id' LIMIT 1";
        $db->query($sql);
        ...
        $sql = "update " . $ecs->table('order_goods') .
                "set goods_status=3 WHERE rec_id = '$rec_id' LIMIT 1";
		/* 如果管理员属于某个办事处，检查该订单是否也属于这个办事处 */
		$sql = "SELECT rec_id FROM " . $ecs->table('order_goods') . " WHERE goods_status <3 and order_id = '$order_id'";
		$rec_ids = $db->getOne($sql);
		if (empty($rec_ids))
		{
...
//                       "WHERE order_id = '$order_id'";
		...
		$sql = "update " . $GLOBALS['ecs']->table('order_goods') .
               "set goods_status=1 WHERE goods_status=0 and order_id = '$order_id' ";
    ...
    elseif ('unpay' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_UNCONFIRMED, SS_UNSHIPPED, PS_UNPAYED, $action_note);
    }
    /* 配货 */
    elseif ('prepare' == $operation)
    {
		...
		$sql = "update " . $ecs->table('order_goods') .
                "set goods_status=3 WHERE order_id = '$order_id'";
        $db->query($sql);
...
function order_list()
{
    ...
    if ($result === false)
    {
		...
		$filter['jiaoyi'] = empty($_REQUEST['jiaoyi']) ? '' : trim($_REQUEST['jiaoyi']);
        ...
        if ($filter['jiaoyi'])
        {
			$filter['jiaoyi']=substr($filter['jiaoyi'], 0,10);
			$filter['jiaoyi']=intval($filter['jiaoyi']);
            $where .= " AND o.order_id = '" . $filter['jiaoyi'] . "'";
        }
        if ($filter['consignee'])
        {
        ...
        if (!is_null($order_amount))
        {
            ...
            if ($log_id > 0)
            {
                ...
                $sql = "UPDATE " . $GLOBALS['ecs']->table('pay_log') .
                        " SET order_amount = '$order_amount' , order_type='" .PAY_YUNFEI.
            ...
            else
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('pay_log') .
                        ...
                        "VALUES('$order_id', '$order_amount', '" . PAY_YUNFEI . "', 0)";
            }
            $GLOBALS['db']->query($sql);
        }
    }

...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            if (register(trim($_POST['email']), trim($_POST['password']), trim($_POST['email'])))
            {
                /* 用户注册成功 */
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                ...
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id,goods_u_info) ".
    ...
    $db->query($sql);
		...
		$sql = "update " . $ecs->table('order_goods') .
                "set goods_status=$goods_status WHERE order_id = '$new_order_id'";
        $db->query($sql);
    ...
    if ($order['order_amount'] <= 0)
    {
        $sql = "SELECT goods_id, goods_name,goods_u_info, goods_number AS num FROM ".
        ...
        foreach ($res AS $row)
        {
            $virtual_goods['virtual_card'][] = array('goods_id' => $row['goods_id'], 'goods_name' => $row['goods_name'], 'goods_u_info' => $row['goods_u_info'], 'num' => $row['num']);
        }
...
elseif ($_REQUEST['step'] == 'update_cart')
{
    ...
    //-- 更新购物车
    /*------------------------------------------------------ */
    if (isset($_POST['goods_u_info']) && is_array($_POST['goods_u_info']))
    {
        foreach ($_POST['goods_u_info'] AS $key => $val)
			{
					$sql = "UPDATE " . $GLOBALS['ecs']->table('cart') .
                            " SET goods_u_info = '$val' WHERE rec_id = $key";
 ...
 * @return  void
 */
function flow_update_cart_beiz($arr)
{

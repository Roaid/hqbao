...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id,g.detail_url,g.tb_nick, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        foreach ($result AS $idx => $row)
        {
			...
			$goods[$idx]['detail_url']   = $row['detail_url'];
			$goods[$idx]['tb_nick']      = $row['tb_nick'];
            $goods[$idx]['name']         = $row['goods_name'];
            $goods[$idx]['brief']        = $row['goods_brief'];
            $goods[$idx]['brand_name']   = isset($goods_data['brand'][$row['goods_id']]) ? $goods_data['brand'][$row['goods_id']] : '';
            $goods[$idx]['goods_style_name']   = add_style($row['goods_name'],$row['goods_name_style']);

			$goods[$idx]['short_name_b']   = sub_str($row['goods_name'], $lenname['best']);
			$goods[$idx]['short_name_n']   = sub_str($row['goods_name'], $lenname['new']);
			$goods[$idx]['short_name_h']   = sub_str($row['goods_name'], $lenname['hot']);
...
function get_recommend_goodsa($cats = '')
{
	$cats = get_children($cats);
    $where = !empty($cats) ? "AND ($cats OR " . get_extension_goods($cats) . ") " : '';

$sql = "SELECT g.* FROM "  . $GLOBALS['ecs']->table('goods') . " AS g  WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".$where ;
$sql .= ' ORDER BY g.last_update DESC LIMIT 10';
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
        $arr[$i]['short_name'] = sub_str($arr[$i]['goods_name'], 14);
        $arr[$i]['url']        = build_uri('goods', array('gid' => $arr[$i]['goods_id']), $arr[$i]['goods_name']);
        $arr[$i]['thumb'] = get_image_path($arr[$i]['goods_id'], $arr[$i]['goods_thumb'],true);
        $arr[$i]['price'] = price_format($arr[$i]['shop_price']);
    }
...
function user_default_goodsc($cats = '')
{
	$cats = get_children($cats);
    $where = !empty($cats) ? "AND ($cats OR " . get_extension_goods($cats) . ") " : '';

$sql = "SELECT g.* FROM "  . $GLOBALS['ecs']->table('goods') . " AS g  WHERE g.is_best = 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".$where ;
$sql .= ' ORDER BY g.last_update DESC LIMIT 5';
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
        $arr[$i]['short_name'] = sub_str($arr[$i]['goods_name'], 20);
        $arr[$i]['url']        = build_uri('goods', array('gid' => $arr[$i]['goods_id']), $arr[$i]['goods_name']);
        $arr[$i]['thumb'] = get_image_path($arr[$i]['goods_id'], $arr[$i]['goods_thumb'],true);
        $arr[$i]['price'] = price_format($arr[$i]['shop_price']);
    }
...
function user_default_goodsa($cats = '')
{
	$cats = get_children($cats);
    $where = !empty($cats) ? "AND ($cats OR " . get_extension_goods($cats) . ") " : '';

$sql = "SELECT g.* FROM "  . $GLOBALS['ecs']->table('goods') . " AS g  WHERE g.is_best = 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".$where ;
$sql .= ' ORDER BY g.last_update DESC LIMIT 8';
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
        $arr[$i]['short_name'] = sub_str($arr[$i]['goods_name'], 12);
        $arr[$i]['url']        = build_uri('goods', array('gid' => $arr[$i]['goods_id']), $arr[$i]['goods_name']);
        $arr[$i]['thumb'] = get_image_path($arr[$i]['goods_id'], $arr[$i]['goods_thumb'],true);
        $arr[$i]['price'] = price_format($arr[$i]['shop_price']);
    }
...
function user_default_goodsb($cats = '')
{
	$cats = get_children($cats);
    $where = !empty($cats) ? "AND ($cats OR " . get_extension_goods($cats) . ") " : '';

$sql = "SELECT g.* FROM "  . $GLOBALS['ecs']->table('goods') . " AS g  WHERE g.is_hot = 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".$where ;
$sql .= ' ORDER BY g.last_update DESC LIMIT 8';
...
function GrabImage($url,$filename="") {
	...
	if($filename=="") 
	{
		...
		$filename="temp/images/".date("dMYHis").$ext;
	}
		ob_start();
		readfile($url);
		$img = ob_get_contents();
 ...
 * 根据淘宝商品URL获取卖家昵称
 */
function taobao_get_nick($taobaoURL)
{
	//http://item.taobao.com/auction/item_detail-0db2-a4b225218cde5cd99c257e19a9715baa.jhtml?is_b=1
	//http://item.taobao.com/auction/item_detail-db2- a6a8eab2d358b6c2a682d481fbd440c5.jhtml
	$regex1 = "/http:\/\/(item|auction1).taobao.com\/auction\/item_detail.(htm|jhtml)?.*(itemID|item_id|item_num_id)=([0-9]{4,10})&.*/";   
	$regex2 = "/http:\/\/item.taobao.com\/auction\/item_detail-.*-([0-9a-zA-Z]{0,32})\..*/";   
	$regex3 = "/http:\/\/(item|auction1).taobao.com\/auction\/item_detail-.*-([0-9a-zA-Z]{0,32})\..*/";
	$regex4 = "/http:\/\/(item|auction1).taobao.com\/item_detail.jhtml\?item_id=([0-9a-zA-Z]{0,32})&.*/";
	$regex5 = "/http:\/\/item.taobao.com\/item.htm\?id=([0-9]{4,10})/";
	$regex6 = "/http:\/\/item.taobao.com\/auction\/item_detail.jhtml\?item_id=([0-9a-zA-Z]{0,32})&.*/";
	$regex7 = "/http:\/\/item.taobao.com\/auction\/item_detail.htm\?item_num_id=([0-9]{0,10}).*/";
	$taobaoURL=strtolower($taobaoURL);
	preg_match($regex1, $taobaoURL,$urlfz);
	if($urlfz[4])
	{
		$shangp['num_iid']=$urlfz[4];
	}
	preg_match($regex2, $taobaoURL,$urlfz);
	if($urlfz[1])
	{
		$shangp['iid']=$urlfz[1];
	}
	preg_match($regex3, $taobaoURL,$urlfz);
	if($urlfz[3])
	{
		$shangp['iid']=$urlfz[3];
	}
	preg_match($regex4, $taobaoURL,$urlfz);
	if($urlfz[2])
	{
		$shangp['iid']=$urlfz[2];
	}	

	preg_match($regex5, $taobaoURL,$urlfz);
	if($urlfz[1])
	{
		$shangp['num_iid']=$urlfz[1];
	}	
	preg_match($regex6, $taobaoURL,$urlfz);
	if($urlfz[1])
	{
		$shangp['iid']=$urlfz[1];
	}
	preg_match($regex7, $taobaoURL,$urlfz);
	if($urlfz[1])
	{
		$shangp['num_iid']=$urlfz[1];
	}
	
	/*if($shangp['iid'] || $shangp['num_iid'])
	{
		$neirong=file_get_contents($taobaoURL);
		$order   = array("\r\n", "\n", "\r","  ");
		$neirong=str_replace($order,'', $neirong);
		preg_match('|<span>掌柜：</span><a ([^<>]+)>([^<>]+)</a>|', $neirong, $matches);
		if($matches[2])
		{
			$shangp['nick'] = 	$matches[2];
		}
		else
		{
			preg_match('/data-nick="([^"]+)"/', $neirong, $con);
			$shangp['nick'] =	$con[1];			
		}
	}*/
	return $shangp;

...
{
    $sql = "SELECT g.* , " .
            "g.goods_price * g.goods_number AS subtotal , gg.goods_img " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') . " AS g ".
			"LEFT JOIN " . $GLOBALS['ecs']->table('goods'). " AS gg ON gg.goods_id = g.goods_id ".
...
//           " WHERE i.user_id = '$user_id' ORDER BY i.add_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['goods_pricef']=price_format($row['goods_price'], false);
		$row['goods_zt']=$GLOBALS['_LANG']['tb'][$row['goods_status']];
		
		if ($row['goods_status']==0)
		{
			$row['action']=$GLOBALS['_LANG']['product_cancel'];	
		}
 ...
 * @return  float   订单总金额
 */
function order_weight($order_id, $include_gift = true)
{
    $sql = "SELECT SUM(goods_zl) " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') .
            " WHERE order_id = '$order_id'";
    if (!$include_gift)
    {
...
function order_fee($order, $goods, $consignee)
{
	...
	$rate = floatval($GLOBALS['_CFG']['invoice_type']['rate'][0]) / 100;
	$total['tax'] = $rate * $total['goods_price'];
	...
	$total['amount_formated_'] = $total['amount']/6.6; // 还需要支付的订单金额
	$total['amount_formated_'] = number_format($total['amount_formated_'], 2, '.', ''); // 还需要支付的订单金额
...
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn,goods_u_info, goods_number, " .
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$beizhu='')
{
    $GLOBALS['err']->clean();

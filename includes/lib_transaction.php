...
//    $sql = "SELECT i.order_id, i.order_sn, i.order_status, i.shipping_status, i.pay_status, i.add_time, " .
//           "(i.goods_amount + i.shipping_fee + i.insure_fee + i.pay_fee + i.pack_fee + i.card_fee + i.tax - i.discount) AS total_fee ,".
//		   "g.goods_id,g.goods_name,g.goods_number,g.goods_price,g.send_number,g.goods_u_info,g.goods_g_info,g.goods_zl ".
//		   "FROM " .$GLOBALS['ecs']->table('order_info'). " AS i ".
//		   "LEFT JOIN " . $GLOBALS['ecs']->table('order_goods'). " AS g ON i.order_id = g.order_id ".
//           " WHERE i.user_id = '$user_id' ORDER BY i.add_time DESC";
  ...
  $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, " .
    ...
    {
		$tuihuo=0;
        if ($row['order_status'] == OS_UNCONFIRMED)//未付款->付款 或 取消
        {	
            $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>  <a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>';
			
			$row['order_status'] = $GLOBALS['_LANG']['ps'][$row['pay_status']] ;
			$tuihuo=1;
        }
        else if ($row['order_status'] == OS_SPLITED)//如果分单
        {
            /* 对配送状态的处理 */
            if ($row['shipping_status'] == SS_SHIPPED)//如果已发货
            {
				//确认收货
			...
			elseif ($row['shipping_status'] == SS_SHIPPED_ING)//如果已经收货
			{
				$row['order_status'] = $GLOBALS['_LANG']['ss'][$row['shipping_status']] ;
			}
			
			elseif ($row['shipping_status'] == SS_SHIPPED_PART)//如果已经收货
			{
				$row['order_status'] = $GLOBALS['_LANG']['ss'][$row['shipping_status']] ;
			}
			
            elseif ($row['shipping_status'] == SS_RECEIVED)//如果已经收货
            {
                @$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
				$row['order_status'] = $GLOBALS['_LANG']['ss'][$row['shipping_status']] ;				
            }
        }
		else if ($row['order_status'] == OS_CONFIRMED)//如果已付款
		{
			  
			  if ($row['pay_status'] == PS_UNPAYED)//如果未付款-邮费 
                {
					$row['order_status'] = $GLOBALS['_LANG']['os']['youfei'] ;//备货中SS_PREPARING
					@$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money_fahuo']. '</a>';
                }
			  elseif ($row['pay_status'] == PS_PAYED)//如果已经收货
			  {
				 	$row['order_status'] = $GLOBALS['_LANG']['ss'][$row['shipping_status']] ;
					$tuihuo=1;
			  }
...
function get_order_detail($order_id, $user_id = 0)
{
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
			...
			$order['order_amount_']=number_format($order['order_amount']/6.6, 2, '.', '');
    ...
    if ($order['add_time'] > 0 )
    {
        $order['add_time'] = sprintf($GLOBALS['_LANG']['confirm_time'], local_date($GLOBALS['_CFG']['time_format'], $order['add_time']));
    }
    else
    {
        $order['add_time'] = '';

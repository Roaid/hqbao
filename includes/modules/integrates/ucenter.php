...
class ucenter extends integrate
{
    ...
    {
        list($uid, $uname, $pwd, $email, $repeat) = uc_call("uc_user_login", array($username, $password,0,0,'','',1));
        ...
        {
            //检查用户是否存在,不存在直接放入用户表
            $user_exist = $this->db->getOne("SELECT user_id FROM " . $GLOBALS['ecs']->table("users") . " WHERE email='$username' AND password = '" . MD5($password) ."'");
			
            $name_exist = $this->db->getOne("SELECT user_id FROM " . $GLOBALS['ecs']->table("users") . " WHERE email='$username'");
            if (empty($user_exist))
            {
                if(empty($name_exist))
                {
    ...
    function set_session1 ($email='')
    {
        ...
        {
            $sql = "SELECT user_id, password, email,user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE email='$email' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);

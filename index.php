...
$smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
	...
	foreach($res1 as $row)
        {
			...
			$arr[$article_id]['description']       = sub_str($row['description'], 66);
    ...
    $sql = 'SELECT article_id, title, author, add_time, file_url, open_type,description' .
               ' FROM ' .$ecs->table('article') .
               ' WHERE (is_open = 1 AND cat_id = 14) ORDER BY article_type DESC, article_id DESC limit 4';
	...
	foreach($res1 as $row)
        {
			...
			$arr2[$article_id]['description']       = sub_str($row['description'], 46);
            $arr2[$article_id]['short_title'] = $GLOBALS['_CFG']['article_title_length'] > 0 ? sub_str($row['title'], 24) : $row['title'];
	...
	$sql = 'SELECT article_id, title, author, add_time, file_url, open_type,description' .
               ' FROM ' .$ecs->table('article') .
               ' WHERE (is_open = 1 AND cat_id = 15) ORDER BY article_type DESC, article_id DESC limit 11';
	...
	foreach($res1 as $row)
        {
            ...
            $arr3[$article_id]['id']          = $article_id;
            $arr3[$article_id]['title']       = $row['title'];
			$arr3[$article_id]['file_url']       = $row['file_url'];
			$arr3[$article_id]['description']       = sub_str($row['description'], 46);
            $arr3[$article_id]['short_title'] = $GLOBALS['_CFG']['article_title_length'] > 0 ? sub_str($row['title'], 12) : $row['title'];
            $arr3[$article_id]['author']      = empty($row['author']) || $row['author'] == '_SHOPHELP' ? $GLOBALS['_CFG']['shop_name'] : $row['author'];
            $arr3[$article_id]['url']         = $row['open_type'] != 1 ? build_uri('article', array('aid'=>$article_id), $row['title']) : trim($row['file_url']);
            $arr3[$article_id]['add_time']    = date($GLOBALS['_CFG']['date_format'], $row['add_time']);
        }
	$smarty->assign('artciles_list3',   $arr3);

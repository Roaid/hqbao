...
if ($action == 'default')
{
	...
	foreach($res1 as $row)
        {
            ...
            $arr[$article_id]['short_title'] = $GLOBALS['_CFG']['article_title_length'] > 0 ? sub_str($row['title'], 22) : $row['title'];
	...
	$smarty->assign('user_default_goodsa',        user_default_goodsa());
	$smarty->assign('user_default_goodsb',        user_default_goodsb());
...
elseif ($action == 'order_detail')
{
	...
	{
		$order['order_status'] = $_LANG['ss'][$order['shipping_status']];
	}
...
elseif ($action == 'tuihuo')
{
	...
	$order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
	$sql = "SELECT user_id, order_id, order_sn , surplus , integral , bonus_id, order_status, shipping_status, pay_status FROM " .$ecs->table('order_info') ." WHERE order_id = '$order_id'";
    $order = $db->GetRow($sql);
    ...
    "set goods_status=4 WHERE rec_id = '$rec_id' LIMIT 1";
    $db->query($sql);
    ...
    if ($order['order_amount'] == 0)
    {
        ...
        elseif ($order['order_status'] == OS_CONFIRMED)
        {
			//leilei
				...
				$_goods = get_order_goodsa(array('order_id' => $order_id, 'order_sn' => $order['order_sn']));
            ...
            $order['confirm_time'] = gmtime();
			$order['shipping_status'] = SS_SHIPPED_ING;
 ...
 * @return array
 */
function get_order_goodsa($order)
{
    $goods_list = array();
    $goods_attr = array();
